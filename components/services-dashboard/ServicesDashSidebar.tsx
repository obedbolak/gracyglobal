...
import {
  ...
  ChevronLeft, ChevronRight, PanelLeft, Crown,
} from "lucide-react";
import { useSubscription } from "@/hooks/useSubscription";

export type ServicesView = "overview" | "services" | "create" | "edit" | "bookings" | "earnings";
...
function NavItems({ activeView, onViewChange, onNavigate }: { activeView: ServicesView; onViewChange: (v: ServicesView) => void; onNavigate?: () => void }) {
  return (
    <>
...
function CollapsedNavItems({ activeView, onViewChange }: { activeView: ServicesView; onViewChange: (v: ServicesView) => void }) {
  return (
    <nav className="px-3 space-y-1">
      {menuItems.map((item) => {
        const Icon = item.icon;
...
export function ServicesDashMobileSidebar({ session, activeView, onViewChange }: { session?: any; activeView: ServicesView; onViewChange: (v: ServicesView) => void }) {
...
export default function ServicesDashSidebar({ collapsed = false, onToggleCollapse, session, activeView, onViewChange }: { collapsed?: boolean; onToggleCollapse?: (c: boolean) => void; session?: any; activeView: ServicesView; onViewChange: (v: ServicesView) => void }) {
  return (
