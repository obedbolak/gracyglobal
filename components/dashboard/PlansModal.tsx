// app/(dashboard)/dashboard/creator/_components/PlansModal.tsx
"use client";
...
interface Props {
  open: boolean;
  onClose: () => void;
  /** Filter to show only plans of this category */
...
export default function PlansModal({ open, onClose, filterCategory }: Props) {
  ...
  const fetchPlans = async () => {
    ...
    try {
      ...
      if (data.success) {
        // Filter to only show relevant category
  ...
  const handleSubscribe = async (planCode: string, planName: string) => {
    ...
    try {
      ...
      // Refresh the page so server-fetched subscription state updates
      onClose();
      router.refresh();
    } catch (e: any) {
