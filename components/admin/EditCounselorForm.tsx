// components/admin/EditCounselorForm.tsx
"use client";
...
export default function EditCounselorForm({
  counselor,
}: EditCounselorFormProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [image, setImage] = useState<string>(counselor.user.image || "");

  const [formData, setFormData] = useState({
    // User fields
    name: counselor.user.name || "",
    phone: counselor.user.phone || "",
    country: counselor.user.country || "CM",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update user info
      const userResponse = await fetch(`/api/users/${counselor.user.id}`, {
        method: "PUT",
      ...
      const counselorResponse = await fetch(`/api/counselors/${counselor.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bio: formData.bio,
      ...
      if (!counselorResponse.ok) {
        ...
        throw new Error(error.error || "Failed to update counselor");
      }
      ...
      console.error(error);
      alert(error.message || "Failed to update counselor");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        `Are you sure you want to delete ${formData.name || "this counselor"}? This cannot be undone.`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Failed to delete counselor");
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-end">
        <button
          ...
          disabled={deleting}
          className="flex items-center gap-2 px-4 py-2 bg-[var(--error-bg)] text-[var(--error-text)] rounded-lg hover:bg-[var(--error-border)] transition-colors disabled:opacity-50"
        >
          <Trash2 className="w-4 h-4" />
          {deleting ? "Deleting..." : "Delete Counselor"}
        </button>
      </div>
      ...
      <div className="glass p-6 rounded-xl space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="glass-input w-full px-4 py-3 opacity-50 cursor-not-allowed"
          />
