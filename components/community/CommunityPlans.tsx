...
export default function CommunityPlans() {
  ...
  async function handleSubscribe(planId: string) {
    ...
    const paymentMethod =
      billing === "session" ? "MOBILE_MONEY_MTN" : "MOBILE_MONEY_MTN";

    const result = await subscribeToPlan(planId, { paymentMethod });
  ...
  function getPrice(plan: (typeof PLANS)[0]) {
    ...
    if (billing === "session")
      return plan.pricePerSession ? fmt(plan.pricePerSession) : "Unlimited";
    return "Free";
  }
  ...
  function getPriceSub(plan: (typeof PLANS)[0]) {
    ...
    if (billing === "session")
      return plan.pricePerSession ? "/ session" : "— no limits";
    return "";
  }
  ...
  function getSaving(plan: (typeof PLANS)[0]) {
    ...
    const saved = Math.round(
      ((plan.priceMonthly - plan.priceAnnual) / plan.priceMonthly) * 100,
    );
    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          >
            Every plan gives you access to Africa's most ambitious community of
            builders, healers, and leaders.
          </p>
          ...
          >
            {(
              ...
              >
                ...
                {id === "annual" && (
                  <span
                    className="absolute -top-2.5 -right-2 text-[9px] font-black px-1.5 py-0.5 rounded-full text-white"
          ...
          {PLANS.map((plan, i) => (
            <motion.div
              ...
              style={
                plan.highlighted
                  ? {
                      boxShadow: `0 20px 60px ${plan.glow}, var(--glass-shadow)`,
            ...
            >
              ...
              <ul className="flex flex-col gap-2.5 flex-1">
                {plan.features.map((f, j) => (
                  ...
                  >
                    <div
                      className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${f.included ? "" : "opacity-40"}`}
                      style={{
        ...
        <motion.p
          ...
          className="text-center text-xs mt-8 font-light"
          style={{ color: "var(--text-disabled)" }}
        >
          All prices in your local currency. Cancel anytime. Prices shown in CFA
          for reference.
        </motion.p>
      </div>
    </section>
